        ...
        if (xhr.readyState == XMLHttpRequest.DONE) {
            ...
            if (callback) {
                try {
                    ...
                    console.log("request:",statusCode,parsedResponse);
...
app.renewToken = function (callback) {
    var currentToken = typeof (app.getSession("token")) == 'object' ? app.getSession("token") : false;
    if (currentToken) {
...
app.bindPaymentConfirmationButton=function(){

    // get card form data
    const card = function () {
        const form = document.querySelector("body.loggedIn .form-container");
        return {
            name: document.querySelector("input#name").value,
            cardnumber: document.querySelector("input#cardnumber").value,
            expirationdate: document.querySelector("input#expirationdate").value,
            securitycode: document.querySelector("input#securitycode").value,
        };
    };

    

    document.querySelector("#shoppingCard #userPaymentConfirmationButton").addEventListener("click", function () {
        // submit User credit card info to validation
        // after response display results
        ...
        app.client.request2({
            path: '/api/shoppingcard/pay/confirm',
            method: 'post',
            ...
            const shoppingCardItems = ((responsePayload || {}).shoppingCard || {}).items || [];
            const amount = ((responsePayload || {}).payload || {}).amount / 100; // usd
            if (ok) {
                //@TODO redirect to pizza flow completed PAGE
                app.paymentState(true);
...
app.paymentState = function( alreadyPayed){
    
    document.querySelector("body .container").style.display = alreadyPayed? 'none': 'block';
    document.querySelector("body .form-container").style.display = alreadyPayed ? 'none' : 'block';
    document.querySelector("#shoppingCard #userPaymentConfirmationButton").style.display = alreadyPayed ? 'none' : 'unset';
    document.querySelector("#shoppingcard-payment").style.display = !alreadyPayed ? 'none' : 'block';
}
...
app.init = function () {
    ...
    app.bindPaymentConfirmationButton();
...
app.loadShoppingCard = function () {
    ...
    var templateItem = `
                                <div class="pizzaItem" data-name"{name}">         
                                    <header>
                                        <h2>{name}</h2>
                                        <img src="{image}" alt="pizza {name}">                                        
                                    </header>   
                                    <summary>
                                        <div class="price">Price {price} {currency}</div>
                                    </summary>
                                </div>`;
    ...
    app.client.request2({
        ...
        const shoppingCardItems = ((paymentIntentData||{}).shoppingCard||{}).items ||[];
        const amount = ((paymentIntentData || {}).payload || {}).amount / 100; // usd
        if (statusCode == 200){       
            // Total
            {
                // sum total from shoppingCard items
                const payValue = shoppingCardItems.reduce(function (a, b) {
                    return a + b.price;
                }, 0);
                if(amount != payValue){ // this should not ocur, but
                    throw new Error("Critical error: aborting");
                }
                const el = document.querySelector("#shoppingCard #shoppingcard-total");
                if(el!=null){
                    el.textContent = payValue+" usd";
                }
            }
            // Address            
            {
                const el = document.querySelector("#shoppingCard #shoppingcard-address");
                if (el != null) {
                    el.textContent = paymentIntentData.payload.shipping.address.line1;
                }                
            }
            helpers.renderArr(wrapper,  shoppingCardItems || [], templateItem);

            // if payment Intent already confirmed 
            const alreadyconfirmed = (paymentIntentData.confirmation||{}).status == 200;
            app.paymentState(alreadyconfirmed);
